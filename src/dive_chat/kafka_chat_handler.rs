...
use kafka::consumer::{
    Consumer, FetchOffset, GroupOffsetStorage, Message, MessageSet, MessageSets,
};
...
impl KafkaChatHandler {
    pub async fn new(hosts: Vec<String>, topic: String) -> Result<Self, KafkaError> {
        let admin_client = AdminClient::from_config(
            &ClientConfig::new().set("bootstrap.servers", hosts.join(",")),
        )
